...
RSpec.shared_examples "a VerifyPin" do
  ...
  it { is_expected.to be_able_to(:read, VerifyPin) }
  it { is_expected.to be_able_to(:verify, VerifyPin) }
...
RSpec.describe "User", :type => :model do
  let(:r_verify_pin) { Wobauth::Role.find_or_create_by(name: 'VerifyPin') }
  subject(:ability) { Ability.new(user) }

  context "with role VerifyPin assigned to user" do
