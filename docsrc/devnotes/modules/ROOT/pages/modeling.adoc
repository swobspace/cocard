...
bin/rails g migration AddInternalToCardTerminal \
  ...
  uptime_reboot:integer \
  slot1_plug_cycles:integer \
  slot2_plug_cycles:integer \
  slot3_plug_cycles:integer \
  slot4_plug_cycles:integer
----
...
bin/rails g migration AddCertificateToCard \
  ...
  cert_subject_postalcode:strin096040g \
...
== model CardCertificate

[source,sh]
----
bin/rails g model CardCertificate \
  card:belongs_to \
  crypt:string \
  cert_ref:string \
  issuer:text \
...
== model KTProxy (TI-Gateway)

[source,sh]
----
bin/rails g scaffold KTProxy \
  card_terminal:belongs_to \
  ti_client:belongs_to \
  uuid:string \
  name:string \
  wireguard_ip:inet \
  incoming_ip:inet \
  ...
  card_terminal_ip:inet \
  card_terminal_port:integer \
  --no-stylesheets --no-javascripts --no-helper
----

== model TIClient (TI-Gateway)

[source,sh]
----
bin/rails g scaffold TIClient \
  connector:belongs_to \
  name:string \
  url:string \
...
== model Oid (Profession OID)

[source,sh]
----
bin/rails g scaffold Oid \
  oid:string:index \
  name:string \
  reference:string \
  --no-stylesheets --no-javascripts --no-helper
