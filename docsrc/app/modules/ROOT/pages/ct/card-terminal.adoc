...
|image::common/card-certificate-button.png[]
|Einlesen von SMC-B/HBA-Kartenzertifikaten
|
...
Viele Felder aus der allgemeinen Information sind bereits in xref:ct/card-terminal-edit.adoc[] beschrieben. Weitere Felder werden aus den Kartenterminal-Informationen extrahiert:
...
Für die Massenpflege von mehreren Kartenterminals gleichzeitig gibt es ein eigenes Formular (siehe xref:ct/idle-messages-edit.adoc[]).
