= Allgemeines zur Oberfläche
:author: Wolfgang Barth
:revdate: 2026-03-01
:imagesdir: ../../images
:experimental: true

== Tabellen

Für die Anzeige von Listen verwendet Cocard überwiegend Tabellen. Als Beispiel dient die Übersicht über Konnektoren in der folgenden Abbildung.

.Übersicht Konnektoren
image::connector/connectors-index.png[]

Die angezeigten Spalten lassen sich über _Column Visibility_ verändern. In vielen Fällen zeigt Cocard in der Standardansicht nicht alle Spalten. Das verwendete Plugin zeigt nun immer nur soviele Spalten an wie Platz auf dem Bildschirm ist. D.h. es ist durchaus möglich, dass Spalten, die über die Sichtbarkeit eingeblendet sein sollen aus Platzgründen doch nicht angezeigt werden. Dann sollte man die angezeigten Spalten reduzieren, bis die gewünschte Spalte doch erscheint. Beim Export als CSV oder Excel werden die sichtbaren Spalten exportiert.

=== Kopfbereich

.Kopfbereich von Tabellen
image::common/table-header.png[]

Neben der Sichtbarkeit via _Column Visibility_ kann man die angezeigten Inhalte sortieren. Dazu klickt man einfach auf die Spaltenüberschrift. Über das Feld _search_ über jeder Spalte filtert man die Einträge.

Filterung, angezeigte Spalten und Sortierung bleiben für einige Stunden erhalten, auch wenn man zwischendurch in einen anderen Bereich von Cocard wechselt. Um die Standardansicht wieder herzustellen nutzt man den Button _Reset_ oberhalb der Tabelle.

=== Tabellenbereich

Die angezeigte Tabelle passt sich der Bildschirmbreite an, die Tabelle ist - im Neudeutsch - _Responsive_. Bei einem schmalen Browser und einer Tabelle wie bei Kartenterminals mit ihren unzähligen Spalten kommt es durchaus vor, dass nicht alle Spalten sichtbar sind, die über _Sichtbare Spalten_ ausgewählt wurden. Wenn sich der Browser nicht weiter verbreitern lässt, muss man daher einige Spalten aus den _Sichtbaren Spalten_ entfernen.

=== Fußbereich

.Fußbereich von Tabellen
image::common/table-bottom.png[]

Im Fußbereich findet man links Informationen zur Anzahl der Einträge und ob ggf. ein Filter aktiv ist. Rechts ist ein Navigator, der zwischen den angezeigten Seiten hin- und her schaltet.

=== Exportfunktionen

Für den Export von Tabellendaten gibt es mehrere Möglichkeiten: Excel, CSV und PDF. Bei Excel und PDF werden immer nur die sichtbaren Spalten exportiert, der CSV-Export enthält dagegen immer alle Spalten. Die Fähigkeiten von PDF sind wegen des festen Seitenaufbaus beschränkt. Wenn man zu viele Spalten exportiert wird der über den Seitenrand hinaus gehende Inhalt abgeschnitten. Je nach Anwendungsfall wird ein Excelexport mit anschließender Konvertierung nach PDF zu besseren Ergebnissen führen.

IMPORTANT: Der Excel-Export setzt keine besonderen Typ für die Spalten, Excel interpretiert daher die ICCSN als besonders große Zahl und stellt diese als Float dar - damit gehen die letzten Stellen verloren. Ähnliches passiert bei bestimmten IP-Adressen. Für den Export von ICCSN und IP-Adressen wählt man daher besser den CSV-Export und importiert die CSV-Datei nach Excel/Calc explizit mit dem Typ `Text` für die Problem-Spalten.