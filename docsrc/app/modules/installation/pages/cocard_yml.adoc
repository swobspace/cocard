...
[source,yml]
----
mail_from: cocard@example.com
mail_to: myticketsystem@example.com
...
IMPORTANT: Der Mailversand erfolgt über den Worker, die Konfigurationsdatei `cocard.yml` muss daher auch beim Worker mit eingebunden sein. Die Beispiele für `docker-compose`, `systemd-quadlets` und Quickstart (`podman pods`) wurden aktualisiert.
...
=== VZD: einen bestimmten Konnektor festlegen
